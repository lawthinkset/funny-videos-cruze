...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🐱🐭 Classic Tom and Jerry Fun!\n\n#tomandjerry #classiccartoons"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🐱🐭 New Tom and Jerry Video!"
        ...
        video = upload.video(
            ...
            name=title or 'Tom and Jerry Classic Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Classic Tom and Jerry Chaos! 🔥"
    title = sys.argv[3] if len(sys.argv) > 3 else "Tom and Jerry"
